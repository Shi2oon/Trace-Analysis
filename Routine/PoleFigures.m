...
function [hw,odf,ODFerror]=PoleFigures(Dirpath,CS,ebsd,grains,plotingpercent)
...
for i=1:length(ebsd.indexedPhasesId)
 ...
 f = figure('visible','off');
    ...
    DirSave = fullfile(DirPoleF,[ebsd.mineralList{ebsd.indexedPhasesId(i)}...
        ' ODF.tif']); saveas(f,DirSave);
    DirSave = fullfile(DirPoleF,[ebsd.mineralList{ebsd.indexedPhasesId(i)}...
        ' ODF.fig']); saveas(f,DirSave); close 
...
%     title('X Y and Z');
    ...
    DirSave = fullfile(DirPoleF,[ebsd.mineralList{ebsd.indexedPhasesId(i)}...
        ' IPF.png']); saveas(f,DirSave); close all
...
%     color = oM.orientation2color(GRAINS(iV).meanOrientation);
    plotPDF(EBSD(GRAINS(iV)).orientations,color,h(iK),'points','all','antipodal','figSize','medium'); 
    hold on
        end
        hold off;
      end
          DirSave = fullfile(DirPoleF,[ebsd.mineralList{ebsd.indexedPhasesId(i)}...
        ' Discrete PF ' xc{iX} '_' num2str(iK) '.tif']);
 ...
 %{
  for iX=3
    ...
    color = oM.orientation2color(EBSD(GRAINS(iV)).orientations);
    %     color = oM.orientation2color(GRAINS(iV).meanOrientation);
    plotIPDF(EBSD(GRAINS(iV)).orientations,color,vector3d.Z,...
        'points','all','antipodal','figSize','medium')
        end
          DirSave = fullfile(DirPoleF,[ebsd.mineralList{ebsd.indexedPhasesId(i)}...
        ' Discrete IPF ' xc{iX} '_' num2str(iK) '.tif']);
    saveas(gcf,DirSave); close
  end
  %}
    % texture analysis ???
...
%% Plotting symmetry
    ...
    plotPDF(odf_model,h(1),'grid','grid_res','antipodal'); 
    plotHKL(CS{i},'projection','eangle','upper','grid_res',...
