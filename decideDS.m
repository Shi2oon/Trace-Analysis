...
elseif sum(strfind(CS.lattice,'hexagonal'))~=0   % need more improvement
...
elseif sum(strfind(CS.lattice,'tetragonal'))~=0 
    ...
    fprintf('Only for FCC, BCC,fct, trigonal and HCP');
end

[sS] = ClearSlip(sS);             [sST] = ClearSlip(sST);
dS   = dislocationSystem(sS);     dST  = dislocationSystem(sST);
dS(dS.isEdge).u  = 1;             dST(dST.isEdge).u  = 1;       % energy of the edge dislocations
dS(dS.isScrew).u = 1 - nu;        dST(dST.isScrew).u = 1 - nu;  % energy of the screw dislocations
end